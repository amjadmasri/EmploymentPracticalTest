...
class ImagesListFragment : BaseFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel = ViewModelProviders.of(this,viewModelProviderFactory).get(ImagesListViewModel::class.java)
        ...
        val imagesObserver = Observer<Resource<PagedList<MvImagesLocal>>> { data ->
            Log.d("amjad","in observing "+data.status)
            Log.d("amjad","in observing "+ (data.data?.size ?: -1))
